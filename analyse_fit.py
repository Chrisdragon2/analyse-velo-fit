...
def estimate_cr_from_width(width_mm):
...
def main_app():
    ...
    with st.sidebar:
        ...
        crr_value = estimate_cr_from_width(tire_width_mm)
    ...
    if uploaded_file is not None:
        ...
        df_blocs = None # Initialiser
        bloc_map = {} # Initialiser
        try:
            df_analyzed = calculate_derivatives(df.copy())
            df_analyzed = identify_and_filter_initial_climbs(df_analyzed, min_pente)
            montees_grouped, df_blocs, bloc_map = group_and_merge_climbs(df_analyzed, max_gap) # Récupère df_blocs et bloc_map ici
        ...
        if montees_grouped is not None and not resultats_df.empty:
            ...
            for index_resultat, df_climb_original in valid_climb_data:
                try:
                    # S'assurer que df_blocs et bloc_map sont passés si create_climb_figure en a besoin (ce n'est plus le cas)
                    fig = create_climb_figure(df_climb_original.copy(), alt_col_to_use, 100, resultats_montées, index_resultat)
                    st.plotly_chart(fig, use_container_width=True, key=f"climb_chart_{index_resultat}") # Ajout d'une clé unique
                except Exception as e:
        ...
        if sprints_df_full.empty and not sprint_error:
            ...
            # Exclure 'Début' (Timestamp) de l'affichage
            ...
            for index, sprint_info in sprints_df_full.iterrows():
                try:
                    ...
                    if not df_sprint_segment.empty:
                     ...
                     st.error(f"Erreur (KeyError) lors du traitement du sprint {index+1}: Clé manquante {ke}. Vérifiez sprint_detector.py.")
                     st.exception(ke)
                except Exception as e:
