...
PUISSANCE_MAX_ECHELLE = 1000.0 

def create_map_figure(df, mapbox_style="carto-positron"):
    """
    Crée une carte Scattermapbox (Version Lignes Chunks)
    avec une VRAIE alternative satellite (ESRI).
    """
    ...
    if has_power_data:
        ...
        min_color_val = 0; max_color_val = PUISSANCE_MAX_ECHELLE; range_color = max_color_val - min_color_val
        colorbar_title = 'Puissance (W)'
    else:
        st.warning("Données de puissance estimée non disponibles. Tracé simple.")
        df_map['plot_color_val'] = 0; colorscale = 'Blues'
    ...
    for name, group in grouped:
        ...
        fig.add_trace(go.Scattermapbox(
            lat=group['position_lat'], lon=group['position_long'],
            mode='lines', line=dict(width=4, color=segment_color_rgb_str),
    ...
    if has_power_data:
        fig.add_trace(go.Scattermapbox(
            ...
            marker=dict(size=0, color=[min_color_val, max_color_val], colorscale=colorscale,
                      showscale=True, colorbar=dict(title=colorbar_title, title_side='right', len=0.7, thickness=20)),
            hoverinfo='none', showlegend=False
        ))

    # --- 5. Mise en forme de la carte (AVEC LOGIQUE SATELLITE) ---
    fig.update_layout(
    ...
    # --- MODIFIÉ : Logique de Style de Carte ---
    if mapbox_style == "satellite_esri":
        # Si "Satellite" est demandé, on utilise une toile blanche
        # et on ajoute le fond de carte ESRI manuellement
        fig.update_layout(
            mapbox_style="white-bg", # Toile vierge
            mapbox_layers=[
                {
                    "below": 'traces',
                    "sourcetype": "raster",
                    "source": [
                        # URL du serveur de tuiles satellite ESRI (gratuit)
        ...
        # Sinon, utiliser les styles intégrés (carto-positron, open-street-map)
