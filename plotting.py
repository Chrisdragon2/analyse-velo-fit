...
def create_climb_figure(df_climb, alt_col_to_use, CHUNK_DISTANCE_DISPLAY, resultats_montées, index):
    ...
    # Ajuster le dtick de la grille en fonction de la taille du chunk
...
# --- MODIFIÉ : create_sprint_figure (pour V-min) ---
def create_sprint_figure(df_sprint_segment, sprint_info, index, display_mode="courbes"):
    ...
    yaxis_vitesse_config = dict(title='Vitesse (km/h)', color='blue', gridcolor='#EAEAEA', side='left')
    yaxis_puissance_config = dict(visible=False)
    
    # Gérer les données vides pour les plages d'axe
    vitesse_max_graph = df_sprint_segment['speed_kmh'].max() * 1.1 + 5 if not df_sprint_segment.empty else 10
    puissance_max_graph = 100 # Défaut
    ...
    if 'estimated_power' in df_sprint_segment.columns:
        ...
        if display_mode == "barres":
             fig.add_trace(go.Bar(x=df_sprint_segment['time_relative_sec'], y=df_sprint_segment['speed_kmh'], name='Vitesse (km/h)', marker_color='blue', yaxis='y1', hovertemplate='<b>Temps:</b> %{x:.1f} s<br><b>Vitesse:</b> %{y:.1f} km/h<extra></extra>'))
        else:
             fig.add_trace(go.Scatter(x=df_sprint_segment['time_relative_sec'], y=df_sprint_segment['speed_kmh'], mode='lines', name='Vitesse (km/h)', line=dict(color='blue', width=2), yaxis='y1', hovertemplate='<b>Temps:</b> %{x:.1f} s<br><b>Vitesse:</b> %{y:.1f} km/h<extra></extra>'))
        yaxis_vitesse_config['range'] = [0, vitesse_max_graph]
